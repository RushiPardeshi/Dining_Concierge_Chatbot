...
def validate_reservation(restaurant):
    ...
    diningdate = restaurant['Date']
    diningtime = restaurant['Time']
    numberpeople = restaurant['NumPeople']
...
def restaurantSQSRequest(requestData):
    ...
    queue_url = 'https://sqs.us-west-2.amazonaws.com/524613615710/myq'
    delaySeconds = 5
    messageAttributes = {
        'cuisine': {
            'DataType': 'String',
...
def make_restaurant_reservation(intent_request):
    ...
    diningdate = try_ex(lambda: intent_request['currentIntent']['slots']['Date'])
    diningtime = try_ex(lambda: intent_request['currentIntent']['slots']['Time'])
    numberpeople = try_ex(lambda: intent_request['currentIntent']['slots']['NumPeople'])
